...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["land-110m.json", new URL("./files/land-110m.json", import.meta.url)],
    ["land-50m.json", new URL("./files/land-50m.json", import.meta.url)],
  ]);
  main.builtin(
    "FileAttachment",
...
See also Jason Davies’ [Rotate the World](https://www.jasondavies.com/maps/rotate/).`;
  ...
  main
    .variable(observer("viewof projectionName"))
    .define("viewof projectionName", ["html"], function (html) {
      return html`<select>
  ...
  main
    .variable(observer("projectionName"))
    .define("projectionName", ["Generators", "viewof projectionName"], (G, _) =>
      G.input(_)
    );
  main
  ...
  main
    .variable(observer("drag"))
    .define("drag", ["d3", "versor"], function (d3, versor) {
  ...
  main
    ...
    .define(
      "projection",
      ["d3", "projectionName"],
      function (d3, projectionName) {
        return d3[projectionName]().precision(0.1);
      }
    );
  main
    ...
    .define(
      "height",
      ["d3", "projection", "width", "sphere"],
      function (d3, projection, width, sphere) {
  ...
  main
    .variable(observer("land50"))
    .define(
      "land50",
      ["FileAttachment", "topojson"],
      function (FileAttachment, topojson) {
        return FileAttachment("land-50m.json")
          .json()
          .then((world) => topojson.feature(world, world.objects.land));
      }
    );
  main
    .variable(observer("land110"))
    .define(
      "land110",
      ["FileAttachment", "topojson"],
      function (FileAttachment, topojson) {
        return FileAttachment("land-110m.json")
          .json()
          .then((world) => topojson.feature(world, world.objects.land));
      }
    );
  main
    .variable(observer("versor"))
    .define("versor", ["require"], function (require) {
  ...
  main.variable(observer("note")).define("note", ["md"], function (md) {
    return md`
---
